...
public class MRefreshView extends LinearLayout {
    // -- header view
    ...
    private MRefreshViewListener mRefreshViewListener;
    // -- footer view
     ...
     * 被刷新的view
     */
    private MRefreshContentView mContentView;
    ...
    private RefreshViewState mState = null;
    /**
    ...
    public MRefreshView(Context context, AttributeSet attrs) {
        ...
        mContentView = new MRefreshContentView();
        mHolder = new RefreshHolder();
    ...
    private void initWithContext(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                    R.styleable.MRefreshView, 0, 0);
            try {
                isHeightMatchParent = a.getBoolean(
                        R.styleable.MRefreshView_isHeightMatchParent, true);
                isWidthMatchParent = a.getBoolean(
                        R.styleable.MRefreshView_isHeightMatchParent, true);
                autoRefresh = a.getBoolean(
                        R.styleable.MRefreshView_autoRefresh, false);
                autoLoadMore = a.getBoolean(
                        R.styleable.MRefreshView_autoLoadMore, true);
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                a.recycle();
    ...
    private void addHeaderView() {
        if (mHeaderView == null) {
            mHeaderView = new MRefreshViewHeader(getContext());
        }
        dealAddHeaderView();
    }
    ...
    private void dealAddHeaderView() {
        if (indexOfChild(mHeaderView) == -1) {
            RefreshUtil.removeViewFromParent(mHeaderView);
    ...
    private void dealAddFooterView() {
        if (indexOfChild(mFooterView) == -1) {
            if (needAddFooterView()) {
                RefreshUtil.removeViewFromParent(mFooterView);
                try {
    ...
    private void attachContentView() {
        mContentView.setContentView(MRefreshView.this.getChildAt(1));
    ...
    private void addFooterView() {
        if (mFooterView == null) {
            mFooterView = new MRefreshViewFooter(getContext());
        }
        dealAddFooterView();
    }
...
//        if(mHolder.mOffsetY!=0)return;
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mHolder.hasHeaderPullDown()) {
                    if (mEnablePullRefresh && !mStopingRefresh && !mPullRefreshing && mHolder.mOffsetY > mHeaderViewHeight) {
                        ...
                        mState = RefreshViewState.STATE_REFRESHING;
                    ...
                    if (!mStopingRefresh) {
                        if (mEnablePullLoad && !isEmptyViewShowing() && needAddFooterView() && !mHasLoadComplete) {
                            ...
                            startScroll(offset, RefreshUtil.computeScrollVerticalDuration(offset, getHeight()));
                        }
                    }
                }
                mLastY = -1; // reset
    ...
    public MRefreshContentView getContentView() {
        return mContentView;
    }
    ...
    public void setHeadMoveLargestDistence(int headMoveDistence) {
        if (headMoveDistence <= 0) {
            int ScreenHeight = RefreshUtil.getScreenSize(getContext()).y;
        ...
        if (isAutoRefresh) {
            ...
            if (mEnablePullRefresh && !mPullRefreshing) {
                if (mHolder.mOffsetY > mHeaderViewHeight) {
                    if (mState != RefreshViewState.STATE_READY) {
                        mHeaderCallBack.onStateReady();
                        mState = RefreshViewState.STATE_READY;
                    }
                } else {
                    if (mState != RefreshViewState.STATE_NORMAL) {
                        mHeaderCallBack.onStateNormal();
                        mState = RefreshViewState.STATE_NORMAL;
                    }
                }
            }
        }
    ...
    private void updateFooterHeight(int deltaY) {
        if (mEnablePullLoad) {
            if (needAddFooterView()) {
                if (isEmptyViewShowing()) {
                    ...
                    if (mState != RefreshViewState.STATE_LOADING) {
                        mFooterCallBack.onStateRefreshing();
                        mState = RefreshViewState.STATE_LOADING;
                    }
                }
        ...
        if (needAddFooterView() || enableRecyclerViewPullUp) {
            if (mEnablePullUpWhenLoadCompleted || !mContentView.hasLoadCompleted()) {
               ...
               /* if (!needAddFooterView() && mContentView.getState() != RefreshViewState.STATE_COMPLETE && autoLoadMore) {
...
//            headerMovePercent = headerMovePercent > 1 ? 1 : headerMovePercent;
    ...
    public void stopRefresh(boolean success) {
        ...
        if (mPullRefreshing == true) {
            ...
            mState = RefreshViewState.STATE_COMPLETE;
    ...
    public void stopLoadMore(boolean hideFooter) {
        mState = RefreshViewState.STATE_FINISHED;
    ...
    private void stopLoadMore(final boolean hideFooter, final int scrollBackDuration) {
        if (needAddFooterView()) {
            if (mPullLoading) {
                mStopingRefresh = true;
                if (mState == RefreshViewState.STATE_COMPLETE) {
    ...
    protected void resetLayout() {
        ...
        if (mHolder.mOffsetY != 0 && !mStopingRefresh) {
            startScroll(-mHolder.mOffsetY, RefreshUtil.computeScrollVerticalDuration(mHolder.mOffsetY, getHeight()));
        }
    }
...
//        mFooterCallBack.onStateRefreshing();
            ...
            if (mScroller.computeScrollOffset()) {
                ...
                if (mHolder.mOffsetY == 0) {
                    ...
                    if (mStopingRefresh && !mPullLoading && !mPullRefreshing) {
                        startScroll(-currentY, RefreshUtil.computeScrollVerticalDuration(currentY, getHeight()));
                    }
                }
            }
        }
    ...
    public void setEmptyView(View emptyView) {
        RefreshUtil.removeViewFromParent(emptyView);
    ...
    public void setRefreshViewListener(MRefreshViewListener l) {
        mRefreshViewListener = l;
        mContentView.setRefreshViewListener(l);
    }
     ...
     */
    public interface MRefreshViewListener {
        /**
    ...
    public static class SimpleRefreshListener implements MRefreshViewListener {
        /**
         * use {@link #onRefresh(boolean)} instead.
         */
        @Deprecated
