...
public abstract class BaseRecyclerAdapter<VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> {
    ...
    public VH onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPES.FOOTER) {
            RefreshUtil.removeViewFromParent(customLoadMoreView);
            VH viewHolder = getViewHolder(customLoadMoreView);
            return viewHolder;
        } else if (viewType == VIEW_TYPES.HEADER) {
            RefreshUtil.removeViewFromParent(customHeaderView);
    ...
    public void onAttachedToRecyclerView(RecyclerView recyclerView) {
        ...
        if (parent != null && parent instanceof MRefreshView) {
            mParent = (MRefreshView) recyclerView.getParent();
    ...
    public void setHeaderView(View headerView, RecyclerView recyclerView) {
        ...
        RefreshUtil.removeViewFromParent(headerView);
        customHeaderView = headerView;
        notifyDataSetChanged();
    }

    public View setHeaderView(int layoutId, RecyclerView recyclerView) {
        if (recyclerView == null) return null;
        Context context = recyclerView.getContext();
        String resourceTypeName = context.getResources().getResourceTypeName(layoutId);
        if (!resourceTypeName.contains("layout")) {
            throw new RuntimeException(context.getResources().getResourceName(layoutId) + " is a illegal layoutid , please check your layout id first !");
        }
        FrameLayout headerview = new FrameLayout(recyclerView.getContext());
        customHeaderView = LayoutInflater.from(context).inflate(layoutId, headerview, false);
        notifyDataSetChanged();
