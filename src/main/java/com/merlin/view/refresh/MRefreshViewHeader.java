...
public class MRefreshViewHeader extends LinearLayout implements IHeaderCallBack {
...
//    private ImageView mOkImageView;
    ...
    public MRefreshViewHeader(Context context) {
        super(context);
        initView(context);
    }
     ...
     * @param attrs
     */
    public MRefreshViewHeader(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }
...
//        mOkImageView = (ImageView) findViewById(R.id.refresh_header_ok);
        mHintTextView = (TextView) findViewById(R.id.refresh_header_hint_textview);
        mHeaderTimeTextView = (TextView) findViewById(R.id.refresh_header_time);
    ...
    public void setRefreshTime(long lastRefreshTime) {
        ...
        if (minutes < 1) {
            refreshTimeText = resources
                    .getString(R.string.refresh_refresh_just_now);
        } else if (minutes < 60) {
            refreshTimeText = resources
                    .getString(R.string.refresh_refresh_minutes_ago);
            refreshTimeText = RefreshUtil.format(refreshTimeText, minutes);
        } else if (minutes < 60 * 24) {
            refreshTimeText = resources
                    .getString(R.string.refresh_refresh_hours_ago);
            refreshTimeText = RefreshUtil.format(refreshTimeText, minutes / 60);
        } else {
            refreshTimeText = resources
                    .getString(R.string.refresh_refresh_days_ago);
            refreshTimeText = RefreshUtil.format(refreshTimeText, minutes / 60 / 24);
        }
...
//        mOkImageView.setVisibility(View.VISIBLE);
        mProgressBar.setVisibility(View.INVISIBLE);
        mHintTextView.setText(success ? R.string.refresh_header_hint_loaded : R.string.refresh_header_hint_loaded_fail);
