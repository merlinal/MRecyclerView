...
public class MRefreshContentView implements OnScrollListener, OnTopRefreshTime, OnBottomLoadMoreTime {
    ...
    private MRefreshView mParent;

    public void setParent(MRefreshView parent) {
        mParent = parent;
    }
    ...
    public void setHolder(RefreshHolder holder) {
        mHolder = holder;
    }
     ...
     * @param container
     */
    public void setContainer(MRefreshView container) {
    ...
    private void setScrollViewScrollListener() {
        if (child instanceof MScrollView) {
            MScrollView scrollView = (MScrollView) child;
            scrollView.setOnScrollListener(mParent, new MScrollView.OnScrollListener() {
                @Override
...
//                        addFooterView(true);
    ...
    private BaseRecyclerAdapter getRecyclerApdater(RecyclerView recyclerView) {
        ...
        if (recyclerView.getAdapter() instanceof BaseRecyclerAdapter) {
            ...
            if (layoutManager != null && layoutManager instanceof GridLayoutManager) {
                ...
                gridLayoutManager.setSpanSizeLookup(new SpanSizeLookup(adapter, gridLayoutManager.getSpanCount()));
            }
    ...
    public void initFooterCallBack(BaseRecyclerAdapter adapter, MRefreshView parent) {
        if (!mSilenceLoadMore) {
            if (adapter != null) {
    ...
    private void doAutoLoadMore(BaseRecyclerAdapter adapter, RecyclerView.LayoutManager layoutManager) {
        if (!mIsLoadingMore && isOnRecyclerViewBottom() && mHideFooter) {
            ...
            setState(RefreshViewState.STATE_NORMAL);
        }
    }

    private boolean isFooterEnable() {
        if (mState != RefreshViewState.STATE_COMPLETE && mParent != null && mParent.getPullLoadEnable()) {
            return true;
        }
        return false;
    }
    ...
    public void startLoadMore(boolean silence, BaseRecyclerAdapter adapter, RecyclerView.LayoutManager layoutManager) {
        ...
        if (!hasLoadCompleted()) {
            ...
            setState(RefreshViewState.STATE_LOADING);
            if (mRefreshViewListener != null) {
    ...
    public void releaseToLoadMore(boolean loadmore) {
        ...
        if (loadmore) {
            if (mState != RefreshViewState.STATE_RELEASE_TO_LOADMORE && !addingFooter) {
                mFooterCallBack.onReleaseToLoadMore();
                setState(RefreshViewState.STATE_RELEASE_TO_LOADMORE);
            }
        } else {
            if (mHideFooter) {
                doReadyState();
                ...
                if (mState != RefreshViewState.STATE_READY) {
                    ...
                    setState(RefreshViewState.STATE_READY);
                }
            }
        }
    }
    ...
    private void doReadyState() {
        if (mState != RefreshViewState.STATE_READY && !addingFooter) {
            mFooterCallBack.onStateReady();
            setState(RefreshViewState.STATE_READY);
        }
    }
...
//        return isBottom();
            ...
            if (!(manager instanceof GridLayoutManager)) {
                ...
                if (footerView != null) {
                    ...
                    if (layoutParams instanceof StaggeredGridLayoutManager.LayoutParams) {
                        RefreshUtil.setFullSpan((StaggeredGridLayoutManager.LayoutParams) layoutParams);
    ...
    public void loadCompleted() {
        ...
        if (mState != RefreshViewState.STATE_COMPLETE) {
            ...
            setState(RefreshViewState.STATE_COMPLETE);
    ...
    private void setState(RefreshViewState state) {
        if (mState != RefreshViewState.STATE_COMPLETE) {
            mState = state;
        }
    }
...
//        mTotalItemCount = 0;
    ...
    private void dealRecyclerViewNotFullScreen() {
        RecyclerView recyclerView = (RecyclerView) child;
        if (onRecyclerViewTop() && !RefreshUtil.isRecyclerViewFullscreen(recyclerView) && child instanceof RecyclerView && mFooterCallBack != null && isFooterEnable()) {
            mFooterCallBack.onStateReady();
            mFooterCallBack.callWhenNotAutoLoadMore(mParent);
            if (!mFooterCallBack.isShowing()) {
                mFooterCallBack.show(true);
    ...
    public void setRefreshViewListener(MRefreshView.MRefreshViewListener refreshViewListener) {
    ...
    public boolean canChildPullUp() {
        if (child instanceof AbsListView) {
            ...
            if (webview instanceof MWebView) {
                return !((MWebView) webview).isBottom();
            } else {
